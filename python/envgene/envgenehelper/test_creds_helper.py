...
check_is_cred_test_data = [
    ...
    ("TEST_CMDB_CREDS_GET_VAULT_NAMESPACE", '${cmdb.creds.get(   "cmdb-creds-get-vault-cred-namespace"  ).namespace}', True),
    ...
    ("SEVERAL_CREDS", "[default]\n        aws_access_key_id = ${creds.get(\"velero-s3-cred\").username}\n        aws_secret_access_key = ${creds.get(\"velero-s3-cred\").password}", True),
...
def test_check_is_cred(param_key, param_value, is_cred):
    assert check_is_cred(param_key, param_value) == is_cred, f"Param {param_key}={param_value} expected cred parsing result should be: {is_cred}"
...
get_cred_list_from_param_test_data = [
    ...
    ("CIP_NETSEASY_SECRET_KEY", "${creds.get('pic-secret-key').secret}", [create_cred_definition("pic-secret-key", CRED_TYPE_SECRET)]),
    ("VMS_CERT_IN_BASE64", '${cmdb.creds.get( "smv-cert" ).secret}', [create_cred_definition("smv-cert", CRED_TYPE_SECRET)]),
    ("TEST_CMDB_CREDS_GET_VAULT_ROLE", '${cmdb.creds.get("cmdb-creds-get-vault-cred-role" ).roleId}', [create_cred_definition("cmdb-creds-get-vault-cred-role", CRED_TYPE_VAULT)]),
    ("TEST_CMDB_CREDS_GET_VAULT_SECRET_ID", '${cmdb.creds.get( "cmdb-creds-get-vault-cred-secret-id").secretId}', [create_cred_definition("cmdb-creds-get-vault-cred-secret-id", CRED_TYPE_VAULT)]),
    ("TEST_CMDB_CREDS_GET_VAULT_PATH", '${cmdb.creds.get( "cmdb-creds-get-vault-cred-path" ).path}', [create_cred_definition("cmdb-creds-get-vault-cred-path", CRED_TYPE_VAULT)]),
    ("TEST_CMDB_CREDS_GET_VAULT_NAMESPACE", '${cmdb.creds.get(   "cmdb-creds-get-vault-cred-namespace"  ).namespace}', [create_cred_definition("cmdb-creds-get-vault-cred-namespace", CRED_TYPE_VAULT)]),
    ("TEST_CMDB_CREDS_GET_PASSWORD_USERNAME", "${cmdb.creds.get('cmdb-creds-get-username-cred').username}", [create_cred_definition("cmdb-creds-get-username-cred", CRED_TYPE_USERPASS)]),
    ("TEST_CMDB_CREDS_GET_PASSWORD_PASSWORD", "${cmdb.creds.get('cmdb-creds-get-password-cred').password}", [create_cred_definition("cmdb-creds-get-password-cred", CRED_TYPE_USERPASS)]),
    ("TEST_CMDB_CREDS_GET_SECRET_PARAM", "${cmdb.creds.get('cmdb-creds-get-secret').secret}", [create_cred_definition("cmdb-creds-get-secret", CRED_TYPE_SECRET)]),
    ("SEVERAL_CREDS", "[default]\n        aws_access_key_id = ${creds.get(\"velero-s3-cred\").username}\n        aws_secret_access_key = ${creds.get(\"velero-s3-cred-password\").password}", [create_cred_definition("velero-s3-cred", CRED_TYPE_USERPASS), create_cred_definition("velero-s3-cred-password", CRED_TYPE_USERPASS)]),
...
def test_get_cred_id_from_cred_macros(param_value, expected_cred_id):
    assert get_cred_id_from_cred_macros(param_value) == expected_cred_id, f"Param {param_value} expected cred parsing result should be: {expected_cred_id}"
...
@pytest.mark.parametrize("cred_macros, expected_cred_id, expected_property", get_cred_id_and_property_from_cred_macros_test_data)
...
def test_get_cred_list_from_param_without_update(param_key, param_value, expected):
    ...
    assert expand_cred_macro_and_return_value(param_key, param_value, credsYaml) == expected, f"Param {param_key}={param_value} expected expanded cred should be: {dump_as_yaml_format(expected)}"
