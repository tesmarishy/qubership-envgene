...
def build_pipeline(params: dict):
    ...
    for env in params['ENV_NAMES'].split("\n"):
        ...
        if any(job in jobs_map for job in jobs_requiring_git_commit) and not params['IS_TEMPLATE_TEST']:
            logger.info(f"inside git commit job {params['DEPLOYMENT-SESSION-ID']}")
            jobs_map["git_commit_job"] = prepare_git_commit_job(pipeline, env, environment_name, cluster_name, params['DEPLOYMENT-SESSION-ID'])
        else:
